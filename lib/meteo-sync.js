...
function createMeteoSync({ apiKey, baseUrl, app, pluginId, log, fetchImpl, radiusNm, limit, now, skipStationsWithoutLiveObs }) {
  ...
  async function syncOnce() {
    ...
    for (const entry of stations) {
      if (skipStationsWithoutLiveObs !== false && entry.identity && entry.identity.has_live_observations === false) {
        skippedNoObs++
        continue
      }
      const context = meteoContextFor(entry.station_uid)
    ...
    if (skippedNoObs) {
      log && log(`${skippedNoObs} station(s) skipped (no live observations, meteoSkipStationsWithoutLiveObs is on)`)
    }

    // No delete step: the stream has no delete API. Stations that fell out of
    // range (or, with the filter above, lost their live reading) are simply
    // not in this update; the client ages their markers out.
    const dropped = [...knownMeteoIds].filter((id) => !nextIds.has(id))
    if (dropped.length) {
      log && log(`${dropped.length} meteo station(s) no longer published — no longer refreshing (client will age them out)`)
    }
